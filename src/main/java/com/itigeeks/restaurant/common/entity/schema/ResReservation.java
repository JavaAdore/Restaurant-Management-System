...
@NamedQueries({@NamedQuery(name = "ResReservation.countAll",
    query = "SELECT COUNT(x) FROM ResReservation x")})
public class ResReservation extends AbstractEntity implements Serializable {
  ...
  // "reservationTypeId" (column "reservation_type_id") is not defined by itself because used as FK
  // in a link
  // "depositTypeId" (column "deposit_type_id") is not defined by itself because used as FK in a
  // link
  // "promoId" (column "promo_id") is not defined by itself because used as FK in a link
  // "reservationStatusId" (column "reservation_status_id") is not defined by itself because used as
  // FK in a link
  // "reservationConfirmationId" (column "reservation_confirmation_id") is not defined by itself
